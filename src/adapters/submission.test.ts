...
describe('Netlify Forms submission', () => {
  ...
  it('URL-encodes form-name, the participant and every answer', () => {
    ...
    expect(body.get('concepts')).toBe('01 Can you check the answer?; 02 Are we checking the same task?; 04 What if the evidence is wrong?');
  ...
  it('succeeds only when the server returns the form’s confirmation page', async () => {
    ...
    for (const status of [404, 405, 500]) {
      const bad = vi.fn().mockResolvedValue(new Response('no', { status }));
      const r = await submitUseCase(input, bad);
      expect(r.ok).toBe(false);
      expect(!r.ok && r.error).toMatch(new RegExp(`error ${status}.*answers are saved`));
    }
  });
  ...
  it('does not trust a bare 200 (e.g. a static server echoing the form file)', async () => {
    ...
    expect(r).toEqual({ ok: false, error: expect.stringMatching(/didn’t confirm it saved/) });
  });
